...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    //TODO
    int start_idx = 0;
    if(read(p_in, &start_idx, 1 * sizeof(int)) == 0){
        fprintf(stderr, "read");
        exit(0);
    }
    int N = 0;
    if(read(p_in, &N, 1 * sizeof(int)) == 0){
        fprintf(stderr, "read");
        exit(0);
    }
    int num_cor = 0;
    int end = start_idx + N - 1;
    if((testing -> num_items) - 1 < start_idx + N - 1){
        end = (testing -> num_items) - 1;
    }
    for(int i = start_idx; i <= end; i++){
        int label = knn_predict(training, &(testing -> images)[i], K, fptr);
        if(label == testing -> labels[i]){
            num_cor += 1;
        }
    }
    if(write(p_out, &num_cor, 1 * sizeof(int)) == 0){
...
double distance_cosine(Image *a, Image *b){
    ...
    double third = 0.0;
    for(int i = 0; i < (a -> sx * a -> sy); i++){
        first += a -> data[i] * b -> data[i];
        second += a -> data[i] * a -> data[i];
        third += b -> data[i] * b -> data[i];
    }
    double result = first /(sqrt(second) * sqrt(third));
    result = 2 * acos(result);
