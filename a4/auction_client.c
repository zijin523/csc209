#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>

#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <netdb.h>


#define BUF_SIZE 128

#define MAX_AUCTIONS 5
#ifndef VERBOSE
#define VERBOSE 0
#endif

#define ADD 0
#define SHOW 1
#define BID 2
#define QUIT 3

/* Auction struct - this is different than the struct in the server program
 */
struct auction_data {
    int sock_fd;
    char item[BUF_SIZE];
    int current_bid;
};

/* Displays the command options available for the user.
 * The user will type these commands on stdin.
 */

void print_menu() {
    printf("The following operations are available:\n");
    printf("    show\n");
    printf("    add <server address> <port number>\n");
    printf("    bid <item index> <bid value>\n");
    printf("    quit\n");
}

/* Prompt the user for the next command 
 */
void print_prompt() {
    printf("Enter new command: ");
    fflush(stdout);
}


/* Unpack buf which contains the input entered by the user.
 * Return the command that is found as the first word in the line, or -1
 * for an invalid command.
 * If the command has arguments (add and bid), then copy these values to
 * arg1 and arg2.
 */
int parse_command(char *buf, int size, char *arg1, char *arg2) {
    int result = -1;
    char *ptr = NULL;
    if(strncmp(buf, "show", strlen("show")) == 0) {
        return SHOW;
    } else if(strncmp(buf, "quit", strlen("quit")) == 0) {
        return QUIT;
    } else if(strncmp(buf, "add", strlen("add")) == 0) {
        result = ADD;
    } else if(strncmp(buf, "bid", strlen("bid")) == 0) {
        result = BID;
    } 
    ptr = strtok(buf, " "); // first word in buf

    ptr = strtok(NULL, " "); // second word in buf
    if(ptr != NULL) {
        strncpy(arg1, ptr, BUF_SIZE);
    } else {
        return -1;
    }
    ptr = strtok(NULL, " "); // third word in buf
    if(ptr != NULL) {
        strncpy(arg2, ptr, BUF_SIZE);
        return result;
    } else {
        return -1;
    }
    return -1;
}

/* Connect to a server given a hostname and port number.
 * Return the socket for this server
 */
int add_server(char *hostname, int port) {
    // Create the socket FD.
    int sock_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (sock_fd < 0) {
        perror("client: socket");
        exit(1);
    }
    
    // Set the IP and port of the server to connect to.
    struct sockaddr_in server;
    server.sin_family = AF_INET;
    server.sin_port = htons(port);
    struct addrinfo *ai;
    
    /* this call declares memory and populates ailist */
    if(getaddrinfo(hostname, NULL, NULL, &ai) != 0) {
        close(sock_fd);
        return -1;
    }
    /* we only make use of the first element in the list */
    server.sin_addr = ((struct sockaddr_in *) ai->ai_addr)->sin_addr;

    // free the memory that was allocated by getaddrinfo for this list
    freeaddrinfo(ai);

    // Connect to the server.
    if (connect(sock_fd, (struct sockaddr *)&server, sizeof(server)) == -1) {
        perror("client: connect");
        close(sock_fd);
        return -1;
    }
    if(VERBOSE){
        fprintf(stderr, "\nDebug: New server connected on socket %d.  Awaiting item\n", sock_fd);
    }
    return sock_fd;
}
/* ========================= Add helper functions below ========================
 * Please add helper functions below to make it easier for the TAs to find the 
 * work that you have done.  Helper functions that you need to complete are also
 * given below.
 */

/* Print to standard output information about the auction
 */
void print_auctions(struct auction_data *a, int size) {
    printf("Current Auctions:\n");

    /* TODO Print the auction data for each currently connected 
     * server.  Use the follosing format string:
     *     "(%d) %s bid = %d\n", index, item, current bid
     * The array may have some elements where the auction has closed and
     * should not be printed.
     */
    for(int i = 0; i < size; i++){
        if(a[i].sock_fd != -1){
            char buf[BUF_SIZE];
            int num_read = read(a[i].sock_fd, buf, strlen(buf));
            if(num_read != -1){
                printf("(%d) %s bid = %d\n", i, a[i].item, a[i].current_bid);
            }
        }
    }
}

/* Process the input that was sent from the auction server at a[index].
 * If it is the first message from the server, then copy the item name
 * to the item field.  (Note that an item cannot have a space character in it.)
 */
void update_auction(char *buf, int size, struct auction_data *a, int index) {
    
    // TODO: Complete this function
    //item bid seconds_remaining
    char *item = strtok(buf, " ");
    if(item == NULL) {
        fprintf(stderr, "ERROR malformed bid: %s", buf);
        exit(1);
    } 
    char *bid = strtok(NULL, " ");
    if(bid == NULL) {
        fprintf(stderr, "ERROR malformed bid: %s", buf);
        exit(1);
    } 
    int b = atoi(bid);
    char *seconds_remaining = strtok(NULL, " ");
    if(seconds_remaining == NULL) {
        fprintf(stderr, "ERROR malformed bid: %s", buf);
        exit(1);
    } 
    int s = atoi(seconds_remaining);
    if(a[index].item[0] == '\0'){
        strncpy(a[index].item, item, strlen(item));  
    }
    a[index].current_bid = b;
    printf("\nNew bid for %s [%d] is %d (%d seconds left)\n", item, index, b, s); 
}

int main(void) {

    char name[BUF_SIZE];

    // Declare and initialize necessary variables
    // TODO
    struct auction_data a[MAX_AUCTIONS];
    int i = 0;
    fd_set allset;
    fd_set rset;
    FD_ZERO(&allset); 
    FD_SET(STDIN_FILENO, &allset);
    int max_fd = STDIN_FILENO;
    // Get the user to provide a name.
    printf("Please enter a username: ");
    fflush(stdout);
    int num_read = read(STDIN_FILENO, name, BUF_SIZE);
    if(num_read <= 0){
        fprintf(stderr, "ERROR: read from stdin failed\n");
        exit(1);
    }
    print_menu();

    // TODO
    name[num_read  - 1] = '\0';
    while(1) {
        print_prompt();
        // TODO
        rset = allset;
        if (select(max_fd + 1, &rset, NULL, NULL, NULL) == -1) {
            perror("client: select");
            exit(1);
        }
        for(int num = 0; num < i; num ++){
            if(a[num].sock_fd != -1){ // not closed yet
                int curr_fd = a[num].sock_fd;
                char client_buf[BUF_SIZE];
                if(FD_ISSET(curr_fd, &rset)){
                    if(read(curr_fd, client_buf, BUF_SIZE) <= 0){
                        FD_CLR(curr_fd, &allset);
                        close(curr_fd);
                        fprintf(stderr, "ERROR: read from stdin failed\n");
                        exit(1);
                    }
                    if(strncmp("Auction closed:", client_buf, 15) == 0){   
                        printf("%s", client_buf);                
                        FD_CLR(curr_fd, &allset);
                        close(curr_fd);
                        a[num].sock_fd = -1;
                    } else {
                        update_auction(client_buf, 0, a, num);
                    }
                }
            }
        }
        if(FD_ISSET(STDIN_FILENO, &rset)){
            char arg1[BUF_SIZE];
            char arg2[BUF_SIZE]; 
            char buf[BUF_SIZE];
            if(read(STDIN_FILENO, buf, BUF_SIZE) <= 0){
                FD_CLR(STDIN_FILENO, &allset);
                close(STDIN_FILENO);
                fprintf(stderr, "ERROR: read from stdin failed\n");
                exit(1);
            }
            int command = parse_command(buf, BUF_SIZE, arg1, arg2);
            if(command ==  0){//Add
                int flag = 0;
                for(int r = 0; r < i; r++){
                    if(a[r].sock_fd == -1){
                        flag = 1;
                        int port = atoi(arg2);
                        int sock_fd = add_server(arg1, port);
                        struct auction_data ad;
                        ad.sock_fd = sock_fd;
                        ad.current_bid = -1;
                        a[r] = ad;
                        if(write(sock_fd, name, strlen(name)) == -1){
                            perror("client: write"); 
                            close(sock_fd);
                            exit(1);
                        }
                        max_fd = sock_fd > max_fd ? sock_fd : max_fd;
                        FD_SET(sock_fd, &allset);
                        FD_SET(sock_fd, &rset);
                    }
                    break;
                }
                if(flag == 0){
                    int port = atoi(arg2);
                    int sock_fd = add_server(arg1, port);
                    struct auction_data ad;
                    ad.sock_fd = sock_fd;
                    ad.current_bid = -1;
                    a[i] = ad;
                    if(write(sock_fd, name, strlen(name)) == -1){
                        perror("client: write"); 
                        close(sock_fd);
                        exit(1);
                    }
                    max_fd = sock_fd > max_fd ? sock_fd : max_fd;
                    FD_SET(sock_fd, &allset);
                    FD_SET(sock_fd, &rset);
                    i += 1;
                }
            }
            if(command ==  1){//Show
                print_auctions(a, i);
            }
            if(command ==  2){//Bid
                int index = atoi(arg1);
                if(write(a[index].sock_fd, arg2, strlen(arg2)) == -1){
                    perror("client: write"); 
                    close(a[index].sock_fd);
                    exit(1);
                }
            }
            if(command ==  3){//Quit
                for(int m = 0; m < i; m++){
                    close(a[m].sock_fd);
                }
                exit(0);
            }
        }
    }
    return 0; // Shoud never get here
}
